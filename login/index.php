 ...
 * Date: 2/29/20
 * Time: 16:18
 * When the client request to this page (GET request), the parameters of
 * `gw_address`,        // The IP address of Gateway in LAN
 * `gw_port`,           // The port number of Gateway in LAN
 * `gw_id`,             // The ID of Gateway in LAN
 * `ip`,                // The IP address of requesting client for authentication
 * `mac`,               // The Mac address of request client for authentication
 * and `url`            // The URL that the client wants to visit. Encoded with URLEncoder
 * should be included.
 * Example: http://wifidog.liwenbin.com/wifidog/login/?gw_address=192.168.1.1&gw_port=2060&gw_id=1708B43AA1E&ip=192.168.1.100&mac=f4:5c:89:a4:be:45&url=http%3A%2F%2Fwww.gstatic.com%2Fgenerate_204
 * After the Login page, the user's request should be redirected to `auth` page with a Token.
 */
...
<div class="login-dark" role="main">
    <form id="auth-form" method="post" class="centered" action="<?php echo $_SERVER['PHP_SELF']; ?>?gw_address=192.168.100.1&gw_port=2060&gw_id=1708B43AA1E&ip=192.168.100.100&mac=f4:5c:89:a4:be:45&url=http%3A%2F%2Fwww.gstatic.com%2Fgenerate_204">
        ...
        <div class="form-group centered">
            ...
            <input class="btn btn-primary btn-block" type="submit" id="requestToConnect" value="Accept &amp; Connect">
            <br>
            <a href="http://192.168.100.1/GetWiFiAccess" download>
...
<script>
    /*
    $('#requestToConnect').onclick(function () {
        $.ajax({
            method: "POST",
            url: "<?php echo $_SERVER['PHP_SELF']; ?>?gw_address=192.168.100.1&gw_port=2060&gw_id=1708B43AA1E&ip=192.168.100.100&mac=f4:5c:89:a4:be:45&url=http%3A%2F%2Fwww.gstatic.com%2Fgenerate_204",
            data: "username="+$('#fname').val()+"&password="+$('#lname'),
